...
<c:if test="${!empty warningMessage}">
    <div class="uportal-channel-warning"><spring:message code="${warningMessage}"/></div>
</c:if>
<table width="100%">
    <tbody><tr>
        ...
        <table width="100%">
            <thead>
                <tr class="uportal-channel-table-header">
                    <th width="15"><input id="${namespace}selectAllSessions" value="selectAllSessions" name="selectAllSessions" type="checkbox"/></th>
                    <th>Session Name</th>
            ...
            <tbody>
                <c:forEach var="session" items="${sessions}" varStatus="loopStatus">
                    ...
                    <tr align="center" class="${loopStatus.index % 2 == 0 ? 'uportal-channel-table-row-odd' : 'uportal-channel-table-row-even'}">
                        <td>
                            <c:if test="${session.currUserCanEdit}"><input value="${session.sessionId}" class="${namespace}deleteSession" name="deleteSession" type="checkbox"/></c:if>
                        </td>
                        <td><a href="${launchSessionUrl}">${session.sessionName}</a></td>
                        <td><fmt:formatDate value="${session.startTime}" pattern="dd-MM-yyyy HH:mm" /></td>
                        <td><fmt:formatDate value="${session.endTime}" pattern="dd-MM-yyyy HH:mm" /></td>
                        <td>
                            <c:if test="${session.currUserCanEdit}"><input value="Edit" name="${session.sessionId}" class="uportal-button" onclick="window.location='${editSessionUrl}'" type="button"></c:if>
                        </td>
                    </tr>
                </c:forEach>
...
<c:choose>
    <c:when test="${fn:length(recordings) gt 0}">
        ...
        <table width="100%">
          <tbody><tr>
            ...
            <form name="deleteRecordings" action="${deleteRecordingActionUrl}" method="POST">
                <td align="right"><input id="dialog-confirm" value="Delete Recording(s)" name="Delete" style="text-transform: none;" class="uportal-button" onclick="javascript:return confirm('Are you sure you wish to delete the recordings(s)?');" type="submit"></td>   
        </tr>
        </tbody></table>
        
        <table width="100%">
            <thead><tr class="uportal-channel-table-header">
                    <th width="15"><input id="${namespace}selectAllRecordings" value="selectAllRecordings" name="Recordings" type="checkbox"/></th>
            ...
            <tbody>
                <c:forEach var="recording" items="${recordings}" varStatus="loopStatus">
                    ...
                    <tr align="center" class="${loopStatus.index % 2 == 0 ? 'uportal-channel-table-row-odd' : 'uportal-channel-table-row-even'}">
                        <td>
                            <c:if test="${recording.currUserCanDelete}"><input value="${recording.recordingId}" class="${namespace}deleteRecording"name="deleteRecording" type="checkbox"/></c:if>
                        </td>
                      <td><a target="_blank" href="${recording.recordingUrl}"><c:out value="${recording.roomName}"/></a></td>
                      <td><fmt:formatDate value="${recording.createdDate}" pattern="dd-MM-yyyy HH:mm" /></td>
                      <td><c:out value="${recording.readableFileSize}"/></td>
                      <td><c:if test="${recording.currUserCanDelete}"><input value="Edit" name="${recording.recordingId}" class="uportal-button" onclick="window.location='${editRecordingUrl}'" type="button"></c:if>
                    </tr>
                </c:forEach>            
            </tbody></table>
    </form>
        ...
        <c:otherwise>
        <br/><b>No recordings available</b><hr/>
    </c:otherwise>
</c:choose>
...
up.jQuery(function() {
    ...
    $('#${namespace}selectAllSessions').click(function () {$('.${namespace}deleteSession').attr('checked', this.checked);});
    $('#${namespace}selectAllRecordings').click(function () {$('.${namespace}deleteRecording').attr('checked', this.checked);});
  });
});
</script>
